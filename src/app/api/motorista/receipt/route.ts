...
export async function GET(request: Request) {
  try {
    ...
    const { data: payment, error } = await supabaseServer
      ...
      .select(`
        *,
        client_profile:cliente_id ( nome, email, celular )
      `)
      .eq('id', paymentId)
    ...
    const clientName = escapeHtml(payment.client_profile?.nome || payment.client_profile?.email || payment.client_profile?.celular || 'Cliente não identificado');
    ...
    const driverIdEsc = escapeHtml(session.id);
    ...
    const html = `
      ...
      <head>
        ...
        <title>Comprovante de Recebimento - AmoPagar</title>
        <style>
      ...
      <body>
        <div class="receipt">
          ...
          <div class="info">
            ...
            <div class="info-row"><span class="info-label">Pagador:</span><span class="info-value">${clientName}</span></div>
            <div class="info-row"><span class="info-label">Valor Recebido:</span><span class="info-value">${formatCurrency(payment.valor)}</span></div>
          ...
          <div class="footer">
            ...
            <p>ID do Recebedor: ${driverIdEsc}</p>
          </div>
        </div>
      </body>
      </html>
