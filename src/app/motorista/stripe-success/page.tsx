    ...
    async function checkStatus() {
      try {
        ...
        if (data.connected) {
          ...
          setTimeout(() => router.push('/motorista/dashboard'), 3000)
        } else if (data.details_submitted) {
          setVerified(true)
          setTimeout(() => router.push('/motorista/dashboard'), 3000)
        } else {
          setError('Sua conta Stripe ainda não está totalmente configurada. Verifique seu email para concluir o processo.')
        }
      } catch {
